define(function(require, exports, module) {
	...
	return {
		...
		createBmapLabel: function(i, b, g, h, f, e) {
			...
			if (g != null) {
				if (map.getZoom() >= 16) {
					c.addEventListener("mouseover", function() {
						c.setZIndex(bMapLabelList.length + 100);
						$("#m_" + i).attr("class", "marker_10")
					});
					c.addEventListener("mouseout", function() {
						c.setZIndex(c._originalZIndex);
						$("#m_" + i).attr("class", c._originalClassName)
					})
				} else {
					c.addEventListener("mouseover", function() {
						c.setZIndex(bMapLabelList.length + 100);
		...
		getNewFloors: function(e, b, c, a, d, m, n, f) {
			...
			var ajaxurl = "";
			if (searchType == 'xinfang') ajaxurl = HOUSE_URL + "map/gethouse?operate=getNewHouses&areaid=" + e + "&keywords=" + (params.keyword != '' ? encodeURIComponent(params.keyword) : '') + "&circleid=" + b + "&projecttype=" + c + "&buildfeature=" + a + "&price=" + d + "&minX=" + min_x + "&maxX=" + max_x + "&minY=" + min_y + "&maxY=" + max_y + '&random=' + Math.random()
			else {
				if (searchType == 'ershoufang') {
					ajaxurl = ESF_URL + "esf_map/gethouse";
				} else if (searchType == 'zufang') {
					ajaxurl = RENT_URL + "zufang_map/gethouse";
				}
				ajaxurl += "?operate=getNewHouses&areaid=" + e + "&circleid=" + b + "&price=" + c + "&roomtype=" + a + "&area=" + d + "&houseage=" + m + "&project=" + n + "&keywords=" + (params.keyword != '' ? encodeURIComponent(params.keyword) : '') + "&minX=" + min_x + "&maxX=" + max_x + "&minY=" + min_y + "&maxY=" + max_y + '&random=' + Math.random()
			}
			$.ajax({
				url: ajaxurl,
				dataType: 'json'
		...
		getKeyHouse: function(k) {
			if (searchType == "xinfang") {
				$.ajax({
					url: HOUSE_URL + "map/getkeyhouse?operate=getNewHouses&keyword=" + encodeURIComponent(k),
					dataType: 'json',
					success: function(h) {
						if (h) {
				...
				if (searchType == 'ershoufang') {
					var ajaxurl = ESF_URL + "esf_map/getkeyhouse";
				} else if (searchType == 'zufang') {
					var ajaxurl = RENT_URL + "zufang_map/getkeyhouse";
				}
				$.ajax({
					url: ajaxurl + "?operate=getNewHouses&keyword=" + encodeURIComponent(k),
					dataType: 'jsonp',
					success: function(h) {
						if (h) {
		...
		openTipFrame: function(c, b) {
			...
			if (searchType == "xinfang") {
				$.ajax({
					type: "GET",
					url: HOUSE_URL + "map/housetip?o=getNewHouse&floorId=" + c,
					dataType: 'json'
				}).done(function(data) {
					if (data != null) {
						var url = "";
						if (data.aliasname != "noAlias") url = "<a href='" + HOUSE_URL + data.aliasname + ".html' target='_blank'>";
						...
						var bbs='';
						if(data.boardId)
							bbs='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="'+data.boardId+'">进入业主论坛</a>';
						var company='';
						if(data.company!="暂无")
							company='<a target="_blank" href="' + HOUSE_URL + 'developer/' + data.companyid + '.html">' + data.company + '</a>';
						else
							company=data.company;
						var html = '<ul id="xfalert"><li>' + url + '<img src="' + data.issuePic + '"></a></li><li><li><span>项目类型：</span>'+data.floorUse+bbs+'</li><li><span>开盘日期：</span>' + data.openQuotation + '</li><li><span>开发商：</span>'+company+'</li><li><span>项目地址：</span>' + data.address + '</li><li><span>售楼电话：</span>' + data.phone + '</li></li></ul>'
						alertM(html, {
							time: 'y',
							btnY: 0,
							width: '480',
				...
				if (searchType == 'ershoufang') {
					var ajaxurl = ESF_URL + "esf_map/houselist";
					var houseurl = ESF_URL;
					var unit = '万';
					var jun='元';
				} else if (searchType == 'zufang') {
					var ajaxurl = RENT_URL + "zufang_map/houselist";
				...
				$.ajax({
					type: "GET",
					cache:false,
					url: ajaxurl + "?o=getHouse&communityid=" + c,
					dataType: 'json'
				}).done(function(data) {
					if (data != null) {
						var price='<h5>按售价筛选:</h5><ul id="map2_filters_price"><li><a priceid="0" href="javascript:;">不限</a></li>';
						var t;
						for(t in data.price){
						...
						room+="</ul>";
						var html='<div class="cf" id="esfalert"><div class="fl">'+price+room+'<ul><li><a target="_blank" class="map2_comm_moreinfo" href="'+COMMUNITY_URL+data.communityid+'.html">查看小区详情&gt;&gt;</a></li><li><a target="_blank" class="map2_comm_fresh" href="'+COMMUNITY_URL+'price/'+data.communityid+'.html">查看小区房价&gt;&gt;</a></li></ul></div><div id="map2_propertys"><div id="map2_info_sort"><span class="fr">排序：<a rel="area" href="javascript:;" class="lightbtn">按面积排序</a><a rel="price" href="javascript:;" class="lightbtn">按价格排序</a></span><span class="span_propnum"> 找到 <em class="prop_num" style="">'+data.nums+'</em> 套房子 </span></div>';
						var list="";
						if(data.nums-0>0){
							list='<div id="property_list"><table width="100%">'
							for(var i=0,l=data.house.length;i<l;i++){
								list+='<tr><td><a href="'+houseurl+data.house[i].hid+'.html" target="_blank"><img src="'+data.house[i].titlepic+'"></a></td><td><a class="title" title="'+data.house[i].title+'" href="'+houseurl+data.house[i].hid+'.html" target="_blank">'+data.house[i].title+'</a><p>'+data.house[i].room+'室'+data.house[i].hall+'厅，'+data.house[i].area+'平米，'+data.house[i].floor+'/'+data.house[i].totalfloor+'层</p></td><td><b class="red">'+data.house[i].price+unit+'</b></td></tr>'}
							list+='</table></div>'
						}else{
						...
						alertM(html+list+page,{
							title:'<a href="'+COMMUNITY_URL+data.communityid+'.html" target="_blank">'+b.communityname+'</a><b>均价：'+b.sellprice+jun+'</b>',
							width:'640',
							time:'y',
							btnY:0,
							of:function(){
								...
								var communityData=function(){
									$("#property_list table").html('<tr><td><img src="<!--#echo var="static"-->images/map/8-0.gif" style="width:220px;height:19px;"></td></tr>');
									$.ajax({
										type: "GET",
										cache:false,
										url:ajaxurl + "?o=getHouse&communityid=" + c + "&price=" + dialogFilter.price + "&roomtype=" + dialogFilter.roomtype + "&curpage=" + dialogFilter.page+ "&areaorder=" + dialogFilter.areaorder + "&priceorder=" + dialogFilter.priceorder,
										dataType: 'json'
									}).done(function(data){
										if(data.curpage==1)
										...
										if(data.house.length>0){
											for(var i=0,l=data.house.length;i<l;i++){
												list+='<tr><td><a href="'+ESF_URL+data.house[i].hid+'.html" target="_blank"><img src="'+data.house[i].titlepic+'"></a></td><td><a class="title" title="'+data.house[i].title+'" href="'+ESF_URL+data.house[i].hid+'.html" target="_blank">'+data.house[i].title+'</a><p>'+data.house[i].room+'室'+data.house[i].hall+'厅，'+data.house[i].area+'平米，'+data.house[i].floor+'/'+data.house[i].totalfloor+'层</p></td><td><b class="red">'+data.house[i].price+unit+'</b></td></tr>'}
										}else
											list='<div id="noprops"><p>很抱歉，没有在该小区找到符合条件的房源。</p><p><b>建议您：</b>重新选择房源筛选条件。</p></div>';
